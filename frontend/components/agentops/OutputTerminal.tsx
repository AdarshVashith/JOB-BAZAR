...
export default function OutputTerminal({ events, status }: Props) {
  ...
  return (
    <div className="rounded-xl border border-[#3A3634] bg-[#1F1D1B] overflow-hidden font-mono shadow-warm">
      {/* VS Code titlebar in Anthropic dark style */}
      <div className="flex items-center bg-[#2A2826] border-b border-[#3A3634] select-none">
        {/* Window dots */}
        ...
        <button
          ...
          className={`px-4 py-2.5 text-xs font-medium border-r border-[#3A3634] transition-colors
              ${tab === "terminal" ? "bg-[#1F1D1B] text-[#F5F3EF] border-t-2 border-t-[#0000CD]" : "text-[#B5AFA5] hover:text-[#F5F3EF]"}`}
        >
          TERMINAL
        </button>
        <button
          ...
          className={`px-4 py-2.5 text-xs font-medium border-r border-[#3A3634] transition-colors
              ${tab === "files" ? "bg-[#1F1D1B] text-[#F5F3EF] border-t-2 border-t-[#0000CD]" : "text-[#B5AFA5] hover:text-[#F5F3EF]"}`}
        >
          OUTPUT{savedFiles.length > 0 ? ` (${savedFiles.length})` : ""}
        </button>

        <div className="ml-auto px-4 flex items-center gap-3 text-xs">
          {status === "running" && (
          ...
          {status === "done" && <span className="text-[#3D8B6E] font-medium">✓ VERIFIED</span>}
          {status === "error" && <span className="text-[#D84C4C] font-medium">✗ ERROR</span>}
          {status === "idle" && <span className="text-[#8A8279]">READY</span>}
          <span className="text-[#8A8279]">sandboxed</span>
        </div>
      </div>

      {/* ── TERMINAL tab ── */}
      {tab === "terminal" && (
        <div className="bg-[#1F1D1B] p-5 min-h-[360px] max-h-[540px] overflow-y-auto text-xs text-[#F5F3EF]">
          <p className="text-[#8A8279] mb-1" suppressHydrationWarning>
            Last login: {startTime.current}
          </p>
          <p className="text-[#8A8279] mb-4">agentops ~ %</p>

          {events.length === 0 ? (
            <div className="flex items-center gap-2">
              <span className="text-[#3B82F6]">agentops</span>
              <span className="text-[#8A8279]">~</span>
              <span className="text-[#F5F3EF]">$</span>
              <span className="text-[#8A8279] animate-pulse">
                awaiting goal...
              </span>
            </div>
          ) : (
