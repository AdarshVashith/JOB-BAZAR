...
export default function SignupPage() {
  ...
  function getPasswordStrength(pw: string): { score: number; label: string; color: string } {
    if (!pw) return { score: 0, label: "None", color: "bg-[#DDD9D1]" };
    let s = 0;
    if (pw.length >= 8) s += 30;
    if (/[A-Z]/.test(pw)) s += 25;
    if (/[0-9]/.test(pw)) s += 25;
    ...
    if (s < 50) return { score: s, label: "Weak", color: "bg-[#D84C4C]" };
    if (s < 80) return { score: s, label: "Medium", color: "bg-[#D97706]" };
    return { score: 100, label: "Strong", color: "bg-[#2D7A5E]" };
  }
  ...
  async function handleSignup(e: React.FormEvent) {
    ...
    if (!agreedToTerms) {
      setError("Please accept the Research & Safety Terms");
      return;
    }
      ...
      <header className="max-w-6xl w-full mx-auto flex items-center justify-between z-10">
        <Link href="/" className="flex items-center gap-2.5 group">
          ...
          <div>
            ...
            <span className="ml-2 px-2 py-0.5 rounded-full text-[10px] font-semibold bg-[#E8EEFF] text-[#2B2FE0] border border-[#C5D4FF]">
              New Workspace
            </span>
          </div>
        </Link>
        ...
        <div className="lg:col-span-6 relative space-y-6">
          ...
          <div className="relative z-10 space-y-5">
            ...
            <h1 className="text-3xl sm:text-5xl font-serif font-normal text-[#1F1915] leading-[1.15]">
              Join the <br />
              <span className="italic text-[#2B2FE0]">orchestration</span>.
            </h1>

            {/* Supporting Copy */}
            <p className="text-sm sm:text-base text-[#4D463E] leading-relaxed max-w-lg">
              Gain full control over custom agent topologies, local vector embeddings, Docker sandboxes, and low-latency LLM routing.
            </p>

            {/* Checkmark Trust Bullets */}
            <div className="space-y-2.5 text-xs text-[#6B6359] pt-2">
              <div className="flex items-center gap-2.5">
                <CheckCircle2 size={16} className="text-[#2D7A5E] flex-shrink-0" />
                <span>Full LangGraph autonomous DAG coordination</span>
              </div>
              <div className="flex items-center gap-2.5">
                <CheckCircle2 size={16} className="text-[#2D7A5E] flex-shrink-0" />
                <span>Persistent memory & BYOK endpoints (Groq, Anthropic, Ollama)</span>
              </div>
              <div className="flex items-center gap-2.5">
                <CheckCircle2 size={16} className="text-[#2D7A5E] flex-shrink-0" />
                <span>Isolated Docker execution sandbox with code verification</span>
              </div>
            </div>
          ...
          <div className="card-editorial p-7 sm:p-8 shadow-warm-lg bg-[#FFFFFE]">
            <div className="mb-6">
              ...
              <p className="text-xs text-[#6B6359] mt-1">
                Initialize your local researcher credentials.
              </p>
            </div>
            ...
            <form onSubmit={handleSignup} className="space-y-3.5">
              {/* Name */}
              <div>
                <label className="block text-xs font-medium text-[#4D463E] mb-1">
                  Full Name
                </label>
                <input
                  type="text"
              ...
              <div>
                ...
                {password.length > 0 && (
                  <div className="mt-2 space-y-1">
                    <div className="h-1 bg-[#EBE8E2] rounded-full overflow-hidden">
                      <motion.div
                        initial={{ width: 0 }}
                    ...
                    <div className="flex justify-between text-[10px] text-[#8A8279]">
                      <span>Strength: {strength.label}</span>
                      <span>8+ chars with mix of letters & symbols</span>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="flex items-center gap-2.5 pt-1">
                <input
                  ...
                  className="w-4 h-4 rounded text-[#2B2FE0] focus:ring-[#2B2FE0] border-[#DDD9D1] cursor-pointer"
                />
                <label htmlFor="terms" className="text-xs text-[#6B6359] cursor-pointer select-none">
                  I agree to the <span className="text-[#2B2FE0] underline">Research & Safety Terms</span>
                </label>
              </div>
              ...
              <button
                type="submit"
                disabled={loading}
                className="btn-primary-blue w-full py-2.5 text-sm font-semibold shadow-sm hover:shadow-warm-md group mt-1"
              >
                {loading ? (
            ...
            <div className="mt-5 pt-4 border-t border-[#EBE8E2] text-center">
